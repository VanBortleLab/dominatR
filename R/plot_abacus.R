...
#'
#' Creates an abacus plot that aims to classify different observations in determined number of groups across variables using an entropy-based method
#'
...
#' @param x_variable A reference column, this is the variable that will be classified. A column with unique gene names or unique index for genes.
#' @param y_variables Columns of interest for visualization purposes.
#' @param title Title for the plot
#' @param percentiles Number of groups intended for classification. If 4 is selected you will obtain the percentiles 0.25, 0.5, 0.75 and 1.00.
#' @param entropy_threshold A numerical value that determines the entropy threshold for visualization.
...
#' @param features_of_interest A vector with the names of genes or index of genes of interest that should be plotted. This names should be present in the x_variable column and have the same format.
#'
#' @returns
#' Returns a list of objects, an abacus classification plot and the respective dataframe
#'
#' @export
#'
#'
plot_abacus = function(data,
  ...
  entropy_threshold =  if (!is.null(entropy_threshold) && length(entropy_threshold) > 0){
    entropy_threshold
  } else {
    c(0, log2(n))
  }
  ...
  plot_heat = data |> ggplot() +
    geom_point(aes(x = X_axis, y = Variable, fill = bin, col = bin), pch = 21, size = point_size) +
    theme_minimal()+
    ggtitle(title) +
    ...
    guides(fill = guide_legend(title = 'Percentile'), color = guide_legend(title = 'Percentile')) +
    xlab('Genes') + ylab('')

  if(single == FALSE){

    plot_heat = plot_heat +
      facet_wrap(~bin, ncol = numb_columns, scales = 'free_x', strip.position = 'right')
  }
